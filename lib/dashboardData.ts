...
async function supabaseQuery(table: string, params: string): Promise<unknown[] | null> {
  try {
...
function buildPeriodLabel(range: string) {
  ...
  const days = range === "7d" ? 7 : range === "qtd" ? 30 : range === "ytd" ? 90 : 30;
...
function applyRange(data: typeof dashboardDemoData, range: string) {
  ...
  ranged.dailySeries = range === "7d" ? (data.dailySeries.slice(-7) as typeof ranged.dailySeries) : data.dailySeries;
...
async function fetchSupabaseMetrics(range: string) {
  ...
  const [keywords, llmRows, gscRows] = await Promise.all([
    supabaseQuery("seo_keyword_proposals", "status=eq.approved&order=score.desc&limit=20&select=keyword,score,source_category,status"),
    supabaseQuery("llm_mention_checks", `run_at=gte.${sinceIso}&order=run_at.desc&limit=50&select=provider,mentioned,run_at`),
    supabaseQuery("gsc_weekly_snapshots", "order=week_start.desc&limit=8&select=week_start,clicks,impressions,avg_position,ctr")
  ]);

  if (!keywords && !llmRows && !gscRows) return null;

  const llmByProvider: Record<string, { total: number; mentioned: number }> = {};
  for (const row of (llmRows ?? []) as Array<{ provider: string; mentioned: boolean }>) {
  ...
  const gscData = (gscRows ?? []) as Array<{ week_start: string; clicks: number; impressions: number; avg_position: number; ctr: number }>;
  return { keywords: keywords ?? [], llmByProvider, latestGsc: gscData[0] ?? null, gscHistory: gscData };
}
...
export async function buildDashboardPayload(range: string, channelParam: string | null) {
  ...
  return {
    ...
    supabase: hasRealData ? {
      ...
      gsc: { latest: supabaseMetrics.latestGsc, history: supabaseMetrics.gscHistory }
    } : null,
    sourceStatus: [
      {
        id: "supabase",
