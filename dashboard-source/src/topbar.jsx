          ...
          <div className="export-wrap" onClick={stop}>
            ...
            {exportOpen && (
              <div className="menu menu-right" onClick={stop}>
                <button className="menu-item" onClick={() => { alert("Export PDF lancé — il vous sera envoyé par email."); setExportOpen(false); }}>
                  <span>Rapport PDF</span><span className="menu-sub">Dashboard complet, à la marque</span>
                </button>
                <button className="menu-item" onClick={() => { alert("Export CSV lancé."); setExportOpen(false); }}>
                  <span>Données CSV</span><span className="menu-sub">Tous les tableaux, cette période</span>
                </button>
                <button className="menu-item" onClick={() => { navigator.clipboard?.writeText(window.location.href); alert("Lien de partage copié dans le presse-papiers."); setExportOpen(false); }}>
                  <span>Copier le lien de partage</span><span className="menu-sub">Lecture seule, expire à 7 jours</span>
                </button>
                <div className="menu-divider" />
                <button className="menu-item" onClick={() => { alert("Programmé : tous les lundis à 9h00 à stakeholders@skstalents.com"); setExportOpen(false); }}>
                  <span>Programmer l'envoi récurrent…</span>
                </button>
              </div>
            )}
          </div>
