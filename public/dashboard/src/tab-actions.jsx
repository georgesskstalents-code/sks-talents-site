...
const INITIAL_ACTIONS = [
  { id: 1, title: "Relancer le retargeting CEO biotech avec nouvelles créas",
    why: "ROAS en baisse de 3.4× à 2.8× — fatigue créative détectée sur 'Notoriété cabinet IG'.",
  ...
  { id: 2, title: "Créer contenu support pour « cabinet recrutement Life Sciences »",
  ...
  { id: 3, title: "Rediriger trafic paid B2B vers /diagnostic",
    why: "La page Diagnostic convertit à 3.8% vs Accueil à 2.8% — gain potentiel de +36% sur conv. paid.",
    source: "Conversion", owner: "Équipe Strategy", due: "Apr 25",
    impact: "high", effort: "low", priority: "P1", status: "todo",
    metrics: [{ label: "Conv. accueil", val: "2.8%" }, { label: "Conv. diag.", val: "3.8%" }] },
  { id: 4, title: "Scaler la campagne LinkedIn dirigeants Life Sciences (+40% budget)",
    why: "ROAS 5.2× avec 42 conv. — plafond du compte Ads non atteint. Fenêtre d'opportunité Série A/B Q2.",
  ...
  { id: 5, title: "Publier fiche métier CMO biotech (best converter)",
    why: "Taux de conv 7.1% sur la page Références, 9.6k sessions. Opportunité SEO autour des rôles dirigeants.",
  ...
  { id: 6, title: "Refonte du formulaire /contact (réduire friction)",
  ...
  { id: 7, title: "Vérifier budget non-brand Google — CPA en baisse",
    why: "CPA a baissé de 9.4% — marge pour scaler la campagne 'recrutement biotech' sans perdre en efficacité.",
  ...
  { id: 8, title: "Mettre à jour les pages sectorielles (refonte a boosté +3 positions)",
    why: "La dernière refonte a fait gagner 3 positions SEO sur 'executive search biotech'. Appliquer à /life-sciences et /animal-health.",
  ...
  { id: 9, title: "Automatiser rapport hebdo COMEX",
    why: "Demande récurrente — un envoi automatique lundi 8h couvrirait 100% des besoins de suivi direction.",
...
function ActionsTab() {
  ...
  React.useEffect(() => {
    ...
    window.fetchDashboardActions?.()
      ...
      .catch(() => {
        if (mounted) {
          setError("Impossible de charger le plan d'action live.");
        }
      })
      .finally(() => {
        if (mounted) {
  ...
  const setStatus = async (id, status) => {
    ...
    const optimistic = actions.map(a => a.id === id ? { ...a, status } : a);
    setActions(optimistic);

    try {
      const saved = await window.updateDashboardAction?.(id, status);
      if (saved) {
        setActions(optimistic.map((item) => item.id === id ? saved : item));
      }
      setError("");
    } catch (err) {
      setActions(previous);
      ...
      {(loading || error) && (
        <section className="card">
          <div className="card-sub">
            {loading ? "Chargement du plan d'action…" : error}
          </div>
        </section>
      )}
